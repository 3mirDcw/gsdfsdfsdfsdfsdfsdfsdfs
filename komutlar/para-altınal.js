...
exports.run = async (client, message, args) => {
  ...
  if (!miktar) return message.reply('<a:reddededee:763326997266038794> Alacağın miktarı yazman gerek!');


  if (miktar.match(x)) return message.reply('<a:reddededee:763326997266038794> Tek zeki sensin zaten!');
if (isNaN(args[0])) return message.channel.send("<a:reddededee:763326997266038794> Lütfen bir sayı gir.")
...
//  let sigara = 1
  ...
  let altinpara = 250
    ...
    if (para < altinpara*miktar) {
      message.channel.send(`<a:reddededee:763326997266038794> ${miktar} Altın alabilmek için yeterli paran görünmüyor. \n Gerekli para **${miktar*altinpara} **`)
 ...
 const i = await db.add(`paracık_${message.member.id}`, -miktar*altinpara)
     ...
     .setColor('RANDOM')
     .setDescription(`<a:dorudoru:763326993135566848> Başarıyla marketten **${miktar}** altın aldınız\n Hesabınızdan eksilen para miktarı: **${miktar*altinpara}TL**`)
     .setTimestamp()
